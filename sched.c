...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	//[1] Set number of ready queues
	num_of_ready_queues = numOfLevels;

	//[2] Create the "quantums" array and initialize it
	quantums = kmalloc(numOfLevels * sizeof(uint8));
	for (int i = 0; i < numOfLevels; i++) {
		quantums[i] = quantumOfEachLevel[i];
	}

	//[3] Create the ready queues and initialize them
	env_ready_queues = kmalloc(numOfLevels * sizeof(struct Env_Queue));
	for (int i = 0; i < numOfLevels; i++) {
		init_queue(&env_ready_queues[i]);
	}

	//[4] Set the CPU quantum by the first level one
	kclock_set_quantum(quantums[0]);
}
...
struct Env* fos_scheduler_MLFQ()
{
	static uint8 curenv_queue_level = 0;
	//TODO: [PROJECT 2026 -[7] CPU Scheduling MLFQ] fos_scheduler_MLFQ()
	// Write your code here, remove the panic and write your code
	...
	//Steps:
	//======
	// [1] If current environment exists, place it in suitable queue
	if (curenv != NULL) {

		if (curenv_queue_level == num_of_ready_queues - 1) {
			// LAST level: Round Robin - stay in same queue
			enqueue(&env_ready_queues[curenv_queue_level], curenv);
		} else {
			// NOT last level: Demote to lower priority (higher index)
			curenv_queue_level++;
			enqueue(&env_ready_queues[curenv_queue_level], curenv);
		}
		curenv = NULL;
	...
	// [2] Search for next env in queues according to priorities
	struct Env* next_env = NULL;
	...
	// [3] If found: set quantum and update global tracker
	if (next_env != NULL) {
		kclock_set_quantum(quantums[selected_level]);
		curenv_queue_level = selected_level;
		return next_env;
	}
