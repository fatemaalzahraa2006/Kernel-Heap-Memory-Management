...
void page_fault_handler(struct Env *curenv, uint32 fault_va)
{
	...
	if (env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
	{
		...
		if (ret == E_NO_MEM)
			panic("No free frames in page fault handler");
		...
		{
			free_frame(frame);
			panic("Failed to map frame in page fault handler");
		}
		...
		if (ret == E_PAGE_NOT_EXIST_IN_PF)
		{
			if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
			{
				...
				if (ret == E_NO_PAGE_FILE_SPACE)
				{
					...
					panic("No page file space for new stack page");
				}
			...
			{
				unmap_frame(curenv->env_page_directory, (void *)fault_va);
				panic("Page fault: illegal memory access at 0x%x", fault_va);
			}
		}
	...
	uint32 victim_va = env_page_ws_get_virtual_address(curenv, victim_index);
	uint32 victim_perms = pt_get_page_permissions(curenv, victim_va);
	...
	if (victim_perms & PERM_MODIFIED)
	{
		...
		if (victim_frame == NULL)
			panic("Victim frame not found");

		int ret = pf_update_env_page(curenv, (void *)victim_va, victim_frame);

		if (ret == E_PAGE_NOT_EXIST_IN_PF)
			panic("Victim page does not exist in page file");
	}
	...
	if (ret == E_NO_MEM)
		panic("No free frames in replacement");
	...
	if (ret != 0)
	{
		...
		panic("Failed to map new frame in replacement");
	}
	...
	if (ret == E_PAGE_NOT_EXIST_IN_PF)
	{
		if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
		{
			...
			{
				unmap_frame(curenv->env_page_directory, (void *)fault_va);
				panic("No page file space for stack page in replacement");
			}
	...
	curenv->page_last_WS_index = victim_index + 1;
	curenv->page_last_WS_index %= curenv->page_WS_max_size;
